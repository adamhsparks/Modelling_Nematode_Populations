...
Import the data using `read_csv()` function from **`readr`**. The data are
...
fields <- data.frame(
  c("Weeks",
    ...
    "Potam",
    "Suneca"),
  c("Number of weeks after wheat sowing",
    "Number of days after wheat sowing",
    "Temperature (˚C) treatment",
    "Average thermal time degree days above 10 ˚C for four soil depths
    (8, 15, 30 and 60 cm)",
    "Log nematode population in the control treatment with no wheat planted",
    "Log nematode population in a susceptible wheat cultivar",
    "Log nematode population in a moderately resistant wheat cultivar",
    "Log nematode population in a susceptible wheat cultivar",
    "Log nematode population in a susceptible wheat cultivar"))
...
column, "Variety". For a more in-depth look at using **`tidyr`** see:
...
plots for your inspection. For a more detailed look at how to interpret these
graphs, please refer to Faraway [-@FARAWAY2002].
...
Using the `summary()` function displays information about the model fit. For
an excellent, detailed look at how to interpret the output from `summary()`, see
The YHAT Blog post,
[http://blog.yhat.com/posts/r-lm-summary.html](http://blog.yhat.com/posts/r-lm-summary.html).
...
$0.70$, we can see here that agrees: $0.6964$. In the original paper,
$P < 0.001$, R reports $p-value: 4.006e-07$, which also agrees.
...
Since the `unplanted_model` object already exists as a prodcut of the linear model, we simply need to use the polynomial model with the unplanted data to create a new object to compare them.
...
`unplanted_poly_model`, `r {summary(unplanted_poly_model)$r.squared}`, than from
the original `unplanted_model`, `r {summary(unplanted_model)$r.squared}`. We can
use the AIC (Akaike information criterion) to get a better feel for the models'
fit to these data. That is, AIC can be used to determine the relative quality of
a model for a _given set of data_, but you cannot compare models for other data
using AIC.
...
linear model has a lower AIC, `AIC(unplanted_model)` than that of the polynomial
model, `r {AIC(unplanted_poly_model)}`, but they are extremely close and the 
$R^2$ value of the polynomial model,
`r {summary(unplanted_poly_model)$r.squared}`, is a bit better than the linear
model's $R^2$, `r {summary(unplanted_model)$r.squared}`, as well.
