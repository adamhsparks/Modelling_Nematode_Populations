...
the data first to see what it looks like.
...
Here we see that the `ln()` transformed data's boxplots show fewer outliers and
tighter range of values. The qq-plots also indicate that it is possible to
conduct a linear regression with these data.

Next, using the `ln()` transformed data, we will fit a smoothed line for each
...
Since the `unplanted_model` object already exists as a prodcut of the linear
