...
subset the data; **`ggplot2`** [-@GGPLOT22009], used for visualising the
data and model fits; and **`broom`** [-@BROOM2017] for tidying the output of
models.
...
| Degree Days | Average thermal time degree days above 10 ˚C for four soil depths (8, 15, 30 and 60 cm) |
| Unplanted   | Log nematode population in the control treatment with no wheat planted |
| Gatcher     | Log nematode population in a susceptible wheat cultivar |
| GS50a       | Log nematode population in a moderately resistant wheat cultivar |
| Potam       | Log nematode population in a susceptible wheat cultivar |
| Suneca      | Log nematode population in a susceptible wheat cultivar |
|             |                                                         |"
...
data format (wide). Using `gather()` from the **`tidyr`** package to convert
from to long format where the varieties are all listed in a single column,
"Variety". For a more in-depth look at **`tidyr`** see:
...
Using `glance()` from **`broom`** inspect the model summary.
