...
if (!require(tidyverse)) {
  install.packages("tidyverse", c(CRAN = "http://cran.rstudio.com"))
  library(tidyverse)
}

if (!require(viridis)) {
  install.packages("viridis", c(CRAN = "http://cran.rstudio.com"))
  library(viridis)
}
```
...
if (!require(knitr)) {
  install.packages("knitr", c(CRAN = "http://cran.rstudio.com"))
  library(knitr)
}
if (!require(kableExtra)) {
  install.packages("kableExtra", c(CRAN = "http://cran.rstudio.com"))
  library(kableExtra)
}
...
fields <- data.frame(
  ...
  c("Number of weeks after wheat sowing",
    ...
    (8, 15, 30 and 60 cm)",
    paste0("Log", footnote_marker_symbol(1), ", `log()`, nematode population in
    the control treatment with no wheat planted"),
    paste0("Log", footnote_marker_symbol(1), ", `log()`, nematode population in a
...
fields %>%
  ...
  kable_styling(bootstrap_options = c("striped",
                                      ...
                                      "responsive")) %>%
  footnote(
    symbol = "For an exploration into the reasons why the data were transformed
              using the natural log `log()`, see the
              [Bonus Material](#Exploring-Why-the-Data-Were-Log-Transformed)
              section"
  )
```
...
interpret these diagnostic plots see the
[Further Reading](Interpreting-Linear-Models-in-R) section.
...
linear model, please refer to the
[Further Reading](#Interpreting-Linear-Models-in-R) section for references that
go into more detail on this matter.
...
`r {round(summary(unplanted_model)$r.squared, 2)}`. In the original paper,
...
[@THOMPSON2015] reported a 61% increase in yield on average from sowing Gatcher,
...
with a reduction occurring due to earlier planting.

The models illustrated here for Gatcher and GS50a were able accurately reflect
the changes in nematode population as a result of degree days, which affected
the nematodes' ability to damage the crop and reduce yield [@THOMPSON2015].
...
[Further Reading](#Further Reading) section for suggested reading on
...
`unplanted_poly_model`, `r {summary(unplanted_poly_model)$r.squared}`, than 
...
linear model has a lower AIC, `r {AIC(unplanted_model)}`, than that of the
polynomial model, `r {AIC(unplanted_poly_model)}`, but they are extremely close
